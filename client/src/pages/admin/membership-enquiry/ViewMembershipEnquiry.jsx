...
const ViewMembershipEnquiry = () => {
  ...
  // ✅ Step 3: Send Profile Visibility → PROFILE_VISIBILITY_SENT
  const handleSendProfileVisibility = async () => {
    if (!window.confirm("Send profile visibility email? Member can choose to make their profile public on the website.")) return;
    try {
      setProcessing(true);
      await sendProfileVisibilityForm(id);
      toast.success("Profile visibility email sent to member!");
      fetchEnquiry();
    } catch (error) {
      toast.error(error.response?.data?.message || "Failed to send profile visibility email.");
    } finally {
      setProcessing(false);
    }
  };
    ...
    return (
      <div className="vme-notfound">
        <div className="vme-notfound__card">
          ...
          <h3 className="vme-notfound__title">Enquiry Not Found</h3>
          <p className="vme-notfound__desc">The membership enquiry you're looking for doesn't exist or has been deleted.</p>
          <button onClick={() => navigate("/admin/membership-enquiries")} className="vme-notfound__btn">
            Back to Enquiries
          </button>
        </div>
      </div>
  ...
  const statusColors = {
    ...
    SEPA_CONSENT_SENT: "vme-status--sepa-sent",
    SEPA_CONSENT_RECEIVED: "vme-status--sepa-received",
    PROFILE_VISIBILITY_SENT: "vme-status--profile-sent",
    PROFILE_COMPLETED: "vme-status--profile-done",
    CHANGES_REQUESTED: "vme-status--changes",
    APPROVED: "vme-status--approved",
    REJECTED: "vme-status--rejected",
  };
  ...
  const currentStep = 
    ...
    enquiry.status === "SEPA_CONSENT_SENT" ? 2 :
    enquiry.status === "SEPA_CONSENT_RECEIVED" ? 3 :
    enquiry.status === "PROFILE_VISIBILITY_SENT" ? 4 :
    enquiry.status === "PROFILE_COMPLETED" ? 5 :
    enquiry.status === "APPROVED" ? 6 : 0;
  ...
  const workflowSteps = [
    ...
    { label: "SEPA Sent", icon: CreditCard },
    { label: "SEPA Received", icon: CheckCircle2 },
    { label: "Profile Sent", icon: UserPlus },
    { label: "Profile Done", icon: CheckCircle2 },
    { label: "Approved", icon: UserCheck },
  ];
  ...
  return (
    <div className="vme-container">
      ...
      {/* ============ HEADER ============ */}
      <div className="vme-header">
        <button onClick={() => navigate("/admin/membership-enquiries")} className="vme-header__back">
          <ArrowLeft size={20} />
          Back to Enquiries
        </button>
        <button onClick={handleDelete} className="vme-header__delete" disabled={processing}>
          <Trash2 size={18} />
          Delete Enquiry
        </button>
      </div>
      ...
      <div className="vme-actions">
        ...
        <div className="vme-actions__grid">
          ...
          {/* ✅ Step 3: Send Profile Visibility */}
          <button
            onClick={handleSendProfileVisibility}
            disabled={processing || enquiry.status !== "SEPA_CONSENT_RECEIVED"}
            className={`vme-actions__btn vme-actions__btn--purple ${
              enquiry.status === "SEPA_CONSENT_RECEIVED" ? "vme-actions__btn--active" : "vme-actions__btn--disabled"
            }`}
          >
            <UserPlus size={18} />
          ...
          <button
            onClick={handleApprove}
            disabled={processing || enquiry.status !== "PROFILE_COMPLETED"}
            className={`vme-actions__btn vme-actions__btn--green ${
              enquiry.status === "PROFILE_COMPLETED" ? "vme-actions__btn--active" : "vme-actions__btn--disabled"
            }`}
          >
            <UserCheck size={18} />
            Step 4: Final Approval
      ...
      <div className="vme-details">
        ...
        <div className="vme-profile">
          ...
          <div className="vme-profile__info">
            ...
            {enquiry.stageName && (
              <p className="vme-profile__stage"><Music size={14} /> {enquiry.stageName}</p>
            )}
            ...
            {enquiry.memberId && (
              <div className="vme-profile__badge vme-profile__badge--green">
                <p className="vme-profile__badge-text">
                  <strong>Member ID:</strong> <code className="vme-profile__badge-code">{enquiry.memberId}</code>
                </p>
              </div>
            )}
            ...
            {/* ✅ Profile Token */}
            {enquiry.profileToken && !enquiry.profileCompleted && (
              <div className="vme-profile__badge vme-profile__badge--purple">
                <p className="vme-profile__badge-text">
                  <strong>Profile Token:</strong> <code className="vme-profile__badge-code">{enquiry.profileToken?.substring(0, 8)}...</code>
                </p>
                <p className="vme-profile__badge-sub">
                  Expires: {enquiry.profileTokenExpiry ? new Date(enquiry.profileTokenExpiry).toLocaleDateString() : "N/A"}
                </p>
              </div>
            )}
          </div>
        ...
        {(enquiry.sepaConsentSent || enquiry.sepaConsentReceived) && (
          <div className="vme-sepa">
            <h3 className="vme-sepa__title"><CreditCard size={18} /> SEPA Direct Debit Mandate</h3>
        ...
        {(enquiry.profileVisibilitySent || enquiry.profileCompleted) && (
          <div className="vme-sepa vme-sepa--profile">
            <h3 className="vme-sepa__title"><UserPlus size={18} /> Profile Visibility</h3>
            
            {enquiry.profileVisibilitySent && (
              <div className="vme-sepa__sent">
                <CheckCircle2 size={16} className="vme-sepa__sent-icon" />
                <span>Profile Email Sent: <strong>{new Date(enquiry.profileVisibilitySentAt).toLocaleString()}</strong></span>
              </div>
            )}
            
            {enquiry.profileCompleted && (
              <div className="vme-sepa__received">
                <div className="vme-sepa__received-header">
                  <CheckCircle2 size={16} className="vme-sepa__received-icon" />
                  <span>Profile Completed: <strong>{new Date(enquiry.profileCompletedAt).toLocaleString()}</strong></span>
                </div>
                <div className="vme-sepa__grid">
                  <div className="vme-sepa__row">
                    <span className="vme-sepa__label">Public Profile:</span>
                    <p className="vme-sepa__value">{enquiry.isPublic ? "✅ Yes" : "❌ No"}</p>
                  </div>
                </div>
              </div>
            )}
        ...
        {/* Manual Status Update */}
        <div className="vme-manual">
          <label className="vme-manual__label"><Clock size={16} /> Manual Status Update (No email sent)</label>
          <select
            value={enquiry.status}
            onChange={(e) => handleStatusUpdate(e.target.value)}
            className="vme-manual__select"
          ...
          >
            ...
            <option value="SEPA_CONSENT_SENT">SEPA CONSENT SENT</option>
            <option value="SEPA_CONSENT_RECEIVED">SEPA CONSENT RECEIVED</option>
            <option value="PROFILE_VISIBILITY_SENT">PROFILE VISIBILITY SENT</option>
        ...
        {/* Personal Information Grid */}
        <div className="vme-section">
          <h3 className="vme-section__title"><User size={16} /> Personal Information</h3>
          <div className="vme-section__grid">
            <InfoCard label="Gender" value={enquiry.gender} icon={<User size={14} />} />
            <InfoCard label="Date of Birth" value={enquiry.dateOfBirth ? new Date(enquiry.dateOfBirth).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) : "-"} icon={<Calendar size={14} />} />
            <InfoCard label="Occupation" value={enquiry.occupation} icon={<Briefcase size={14} />} />
        ...
        {enquiry.biography && (
          <div className="vme-section">
            <h3 className="vme-section__title"><BookOpen size={16} /> Biography</h3>
            <div className="vme-section__bio">{enquiry.biography}</div>
          </div>
        )}
        ...
        {enquiry.remarks && (
          <div className="vme-section vme-section--remarks">
            <h3 className="vme-section__title vme-section__title--orange"><AlertCircle size={16} /> Remarks</h3>
            <div className="vme-section__remarks">{enquiry.remarks}</div>
          </div>
        )}
...
const InfoCard = ({ label, value, icon, highlight }) => (
  <div className={`vme-info ${highlight ? "vme-info--highlight" : ""}`}>
    <p className="vme-info__label">{icon}{label}</p>
    <p className="vme-info__value">{value || "-"}</p>
  </div>
);
