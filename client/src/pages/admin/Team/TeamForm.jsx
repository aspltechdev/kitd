...
const TeamForm = ({
  ...
  const isActive = watch("isActive", true);
  const level = watch("level", "MEMBER");
  ...
  useEffect(() => {
    if (initialValues && Object.keys(initialValues).length > 0) {
      reset({
        name: initialValues.name || initialValues.fullName || "",
        designation: initialValues.designation || "",
        biography: initialValues.biography || initialValues.bio || "",
        level: initialValues.level || "MEMBER",
        email: initialValues.email || "",
        mobile: initialValues.mobile || initialValues.phone || "",
        stageName: initialValues.stageName || "",
        danceForm: initialValues.danceForm || initialValues.danceStyle || "",
        ...
        sortOrder: initialValues.sortOrder || initialValues.displayOrder || 0,
        isPublic: initialValues.isPublic !== undefined ? initialValues.isPublic : false,
        isActive: initialValues.isActive !== undefined ? initialValues.isActive : true,
      });

      if (initialValues.image || initialValues.photo) {
        const imgUrl = initialValues.image || initialValues.photo;
        setPreview(`${IMAGE_BASE_URL}/uploads/team/${imgUrl}`);
        setFileName(imgUrl);
      }
    }
  }, [initialValues, reset]);
  ...
  const handleDrop = (e) => {
    ...
    setDragOver(false);
    const files = e.dataTransfer.files;
    if (files.length > 0) setValue("image", files);
  };

  const clearFile = () => { setPreview(null); setFileName(""); setValue("image", []); };

  const levelOptions = [
    { value: "BOARD", label: "🏛️ Board of Directors" },
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="team-form">
      <div className="team-form__grid">
        ...
        <div className="team-form__main">
          ...
          <div className="form-section visibility-section">
            ...
            >
              {isPublic ? (
                <><Eye size={20} /><span>Visible on Website</span><ToggleRight size={22} /></>
              ) : (
                <><EyeOff size={20} /><span>Hidden from Website</span><ToggleLeft size={22} /></>
              )}
            </button>
          </div>

          {/* ============ HIERARCHY LEVEL ============ */}
          <div className="form-section level-section">
            <h3 className="form-section__title">
              <Building2 size={20} /> Team Level
            </h3>
            <div className="level-options">
              {levelOptions.map((option) => (
                <label
                  key={option.value}
                  className={`level-option ${level === option.value ? "active" : ""}`}
                >
                  <input
                    type="radio"
                    {...register("level")}
          ...
          <div className="form-section">
            <h3 className="form-section__title"><User size={20} /> Personal Information</h3>

            <div className="form-group">
              <label className="form-group__label">Full Name <span className="required">*</span></label>
              <div className="form-group__input-wrapper">
                <User size={18} className="input-icon" />
                <input {...register("name", { required: "Full name is required" })} className={`form-group__input ${errors.name ? "error" : ""}`} placeholder="Enter full name" disabled={loading} />
              </div>
            ...
            <div className="form-group form-group--inline">
              <div className="form-group__half">
                <label className="form-group__label">Designation <span className="required">*</span></label>
                <div className="form-group__input-wrapper">
                  <Briefcase size={18} className="input-icon" />
                  <input {...register("designation", { required: "Designation is required" })} className={`form-group__input ${errors.designation ? "error" : ""}`} placeholder="e.g., President, Teacher" disabled={loading} />
                </div>
                {errors.designation && <p className="form-group__error">{errors.designation.message}</p>}
              </div>
              <div className="form-group__half">
                <label className="form-group__label">Stage Name</label>
                <div className="form-group__input-wrapper">
                  <Music size={18} className="input-icon" />
                  <input {...register("stageName")} className="form-group__input" placeholder="Stage name" disabled={loading} />
                </div>
              </div>
            </div>
            ...
            <div className="form-group form-group--inline">
              <div className="form-group__half">
                ...
                <div className="form-group__input-wrapper">
                  <Mail size={18} className="input-icon" />
                  <input type="email" {...register("email")} className="form-group__input" placeholder="Email address" disabled={loading} />
                </div>
              </div>
              <div className="form-group__half">
                ...
                <div className="form-group__input-wrapper">
                  <Phone size={18} className="input-icon" />
                  <input {...register("mobile")} className="form-group__input" placeholder="Phone number" disabled={loading} />
                </div>
              </div>
            </div>
            ...
            <div className="form-group form-group--inline">
              <div className="form-group__half">
                <label className="form-group__label">Dance Form</label>
                <div className="form-group__input-wrapper">
                  <Music size={18} className="input-icon" />
                  <input {...register("danceForm")} className="form-group__input" placeholder="Dance style" disabled={loading} />
                </div>
              </div>
              <div className="form-group__half">
                ...
                <div className="form-group__input-wrapper">
                  ...
                  <input {...register("city")} className="form-group__input" placeholder="City, Country" disabled={loading} />
                </div>
              </div>
            </div>
          ...
          <div className="form-section">
            ...
            <div className="social-links-grid">
              <div className="form-group__input-wrapper">
                <FaLinkedinIn size={18} className="input-icon social-icon linkedin" />
                <input {...register("socialLinks.linkedin")} className="form-group__input" placeholder="LinkedIn URL" disabled={loading} />
              </div>
              <div className="form-group__input-wrapper">
              ...
              <div className="form-group__input-wrapper full-width">
                <Globe size={18} className="input-icon" />
                <input {...register("socialLinks.website")} className="form-group__input" placeholder="Personal Website" disabled={loading} />
              </div>
            </div>
          </div>
          ...
          <div className="form-section">
            ...
            <div className="form-group form-group--inline">
              <div className="form-group__half">
                <label className="form-group__label">Sort Order</label>
                <div className="form-group__input-wrapper">
                  <Hash size={18} className="input-icon" />
                  <input type="number" {...register("sortOrder")} className="form-group__input" placeholder="0" disabled={loading} />
                </div>
              </div>
              <div className="form-group__half">
                <label className="form-group__label">Status</label>
        ...
        <div className="team-form__media">
          <div className="media-section">
            <div className="media-section__header">
              <div className="media-section__header-left"><ImageIcon size={20} /><h3>Profile Image</h3></div>
            </div>
            <div className={`media-dropzone ${dragOver ? "drag-over" : ""} ${preview ? "has-preview" : ""}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
              {!preview ? (
                <div className="media-dropzone__content">
                  ...
                  <p className="dropzone-text">Drag & drop an image here</p>
                  <span className="dropzone-hint">or click to browse</span>
                  <input type="file" accept="image/*" {...register("image")} className="media-dropzone__input" disabled={loading} id="image-upload" />
                  <label htmlFor="image-upload" className="dropzone-btn">Choose Image</label>
                </div>
              ) : (
                <div className="media-preview">
                  <div className="media-preview__container">
      ...
      <div className="team-form__actions">
        <button type="submit" disabled={loading} className="btn btn--primary btn--submit">
          {loading ? <><span className="spinner-btn"></span>{isEdit ? "Updating..." : "Creating..."}</> : <><Check size={18} />{isEdit ? "Update Member" : "Add Member"}</>}
        </button>
      </div>
    </form>
  );
