...
import { useNavigate, Link } from "react-router-dom";
import toast from "react-hot-toast";
import { ArrowLeft, X, AlertCircle, UserPlus } from "lucide-react";
...
const AddMember = () => {
  ...
  // ✅ Generate unique Member ID using timestamp
  const generateUniqueMemberId = () => {
    const year = new Date().getFullYear();
    const timestamp = Date.now().toString().slice(-6);
    const random = Math.floor(Math.random() * 99) + 1;
    const suffix = String(random).padStart(2, '0');
    return `KITD-${year}-${timestamp}${suffix}`;
  };

  useEffect(() => {
    setGeneratedMemberId(generateUniqueMemberId());
  }, []);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      formData.append("memberId", generatedMemberId);
      formData.append("fullName", data.fullName);
      ...
      // Membership Details
      formData.append("joinedDate", data.joinedDate || new Date().toISOString().substring(0, 10));
      formData.append("expiryDate", data.expiryDate || "");
      ...
      // If duplicate ID, regenerate and notify
      if (message.includes("memberId") || message.includes("already exists")) {
        setGeneratedMemberId(generateUniqueMemberId());
        toast.error("ID conflict. New ID generated. Please try submitting again.");
      } else {
        toast.error(message);
      }
    } finally {
  ...
  const handleRegenerateId = () => {
    setGeneratedMemberId(generateUniqueMemberId());
    toast.success("New Member ID generated!");
  };
  ...
  return (
    <div className="add-member-page">
      <div className="add-member-page__container">
        ...
        <div className="add-member-page__header">
          <div className="add-member-page__header-left">
            ...
            <div className="add-member-page__header-title">
              ...
              <div>
                <h1 className="add-member-page__title">Add Member</h1>
                <p className="add-member-page__subtitle">Register a new association member</p>
              </div>
            </div>
          </div>
          <div className="add-member-page__header-actions">
        ...
        {/* Member ID Banner */}
        <div className="member-id-banner">
          <div className="member-id-banner__content">
            <span className="member-id-banner__label">Member ID:</span>
            <code className="member-id-banner__value">{generatedMemberId}</code>
          </div>
          <button type="button" onClick={handleRegenerateId} className="member-id-banner__regenerate" disabled={loading}>
            🔄 Regenerate
          </button>
        </div>
        ...
        <div className="add-member-page__form-card">
          <div className="form-card__header">
            <div className="form-card__header-icon"><UserPlus size={24} /></div>
            <div>
              <h2 className="form-card__title">Member Details</h2>
              <p className="form-card__subtitle">Fill in the information below to register a new member</p>
            </div>
          </div>
        ...
        <div className="add-member-page__tips">
          <div className="tips-card">
            ...
            <ul className="tips-list">
              <li>Member ID is auto-generated (unique timestamp-based)</li>
              <li>Use a valid email address for communication</li>
              <li>Select the appropriate membership type</li>
              <li>Upload a clear profile photo (optional)</li>
              <li>SEPA details enable payment processing</li>
            </ul>
          </div>
        </div>
