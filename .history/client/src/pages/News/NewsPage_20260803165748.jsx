...
// Import images
import acthero from "../../assets/banner4.jpeg";
import contactcta from "../../assets/newscta.png";
import abt1 from "../../assets/abt1.png";
import abt2 from "../../assets/abt2.png";

const fallbackImages = [abt1, abt2];
...
const NewsPage = () => {
  ...
  if (loading) {
    return (
      <div className="np-news">
        <div className="np-news__loading">
          <div className="np-news__spinner" />
          <p>Loading news...</p>
        </div>
      </div>
    );
  ...
  return (
    <>
      <Helmet>
        <title>News & Updates | KITD - Classical Indian Dance Germany</title>
        <meta name="description" content="Stay informed with the latest announcements, event highlights, and community updates from KITD across Germany." />
      </Helmet>

      <div className="np-news">

        {/* HERO */}
        <section className="np-news__hero">
          <div className="np-news__hero-bg">
            <img src={acthero} alt="KITD News" loading="eager" />
            <div className="np-news__hero-overlay" />
            <div className="np-news__hero-gradient" />
          </div>
          <div className="np-news__hero-container">
            <div className="np-news__hero-content">
              <span className="np-news__hero-tag">NEWS & UPDATES</span>
              <h1>
                Keeping the KITD <span className="np-news__hero-title-accent">Community Connected</span>
              </h1>
              <p className="np-news__hero-desc">
                Stay informed with the latest announcements, event highlights, and community updates from KITD across Germany.
              </p>
            </div>
          </div>
        </section>
        ...
        {/* SEARCH */}
        <section className="np-news__search">
          <div className="np-news__container">
            <div className="np-news__search-wrapper">
              <div className="np-news__search-bar">
                <span className="np-news__search-icon">🔍</span>
                <input
                  type="text"
                  placeholder="Search news articles..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="np-news__search-input"
                />
                {searchQuery && (
                  <button className="np-news__search-clear" onClick={() => setSearchQuery("")}>
                    <X size={16} strokeWidth={1.5} />
                  </button>
                )}
              </div>
              <span className="np-news__count">
        ...
        {/* NEWS GRID */}
        <section className="np-news__grid">
          <div className="np-news__container">
            {filteredNews.length === 0 ? (
              <div className="np-news__empty">
                <Newspaper size={48} strokeWidth={1.5} />
                <h3>No articles found</h3>
                <p>Try adjusting your search.</p>
              </div>
            ) : (
              <div className="np-news__grid-inner">
                {filteredNews.map((item, index) => {
                  const imageUrl = getImageUrl(item);
                  return (
                    <article 
                      key={item.id || index}
                      className={`np-news__card ${hoveredCard === index ? 'np-news__card--hovered' : ''}`}
                      onMouseEnter={() => setHoveredCard(index)}
                      onMouseLeave={() => setHoveredCard(null)}
                      style={{ transitionDelay: `${(index % 6) * 0.05}s` }}
                    >
                      <Link to={`/news/${item.id}`} className="np-news__card-link">
                        <div className="np-news__card-image">
                          <img 
                            src={imageUrl} 
                            alt={item.title || "News article"} 
                            loading="lazy"
                          />
                          <div className="np-news__card-overlay" />
                        </div>
                        <div className="np-news__card-content">
                          {item.createdAt && (
                            <div className="np-news__card-date">
                              <Calendar size={14} strokeWidth={1.5} />
                              <span>{formatDate(item.createdAt)}</span>
                            </div>
                          )}
                          <h3 className="np-news__card-title">{item.title || "Untitled"}</h3>
                          {item.description && (
                            <p className="np-news__card-desc">
                              {item.description.length > 120 
                                ? `${item.description.substring(0, 120)}...` 
                                : item.description}
                            </p>
                          )}
                          <span className="np-news__card-read">
        ...
        {/* CTA */}
        <section className="np-news__cta">
          <div className="np-news__cta-bg">
            <img src={contactcta} alt="KITD Community" loading="lazy" />
            <div className="np-news__cta-overlay" />
          </div>
          <div className="np-news__container">
            <div className="np-news__cta-wrapper">
              <h2>Stay Connected with KITD</h2>
              <p>Subscribe to receive the latest news, event announcements and community updates.</p>
              <div className="np-news__cta-buttons">
                <Link to="/membership" className="np-news__cta-btn np-news__cta-btn--primary">
                  <Mail size={18} strokeWidth={1.5} /> Become a Member
                </Link>
                <Link to="/contact" className="np-news__cta-btn np-news__cta-btn--secondary">
                  Contact KITD
                </Link>
              </div>
            </div>
